// forked from design-sync lib/source-kit.mjs — synth-entry mode has no
// per-file exclude knob (componentSrcMap:null only hides a name from the
// component list, it doesn't stop the file from being `export * from`'d into
// the synthesized entry), so ENTRY_EXCLUDE below drops specific source files
// from the entry before it's written. See cfg.libOverrides for why.
//
...
// Files that must never enter the synthesized entry: their transitive imports
// (here, @react-three/fiber -> scheduler, via canvas-reveal-effect.tsx pulled
// in by card-spotlight[-demo].tsx too) evaluate at bundle-load time and break
// the whole shared IIFE, not just their own component. See NOTES.md.
const ENTRY_EXCLUDE = /\/(canvas-reveal-effect|faulty-terminal|card-spotlight|card-spotlight-demo)\.tsx$/;
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !ENTRY_EXCLUDE.test(p));
    entry = join(OUT, '.pkg-entry.mjs');
    // `export * from` never re-exports a default export (ES module spec) —
    // some components here ship as `export default Foo`/`export default
    // function Foo` only (no named export), so window.<GLOBAL>.Foo would
    // silently be missing ([BUNDLE_EXPORT] catches it). Cheap regex recovery
    // (no need for a full ts-morph pass just for this) re-exposes the
    // default under its declared name alongside the barrel `export *`.
    const lines = comps.flatMap((p) => {
      const out = [`export * from ${JSON.stringify(p)};`];
      const m = /export\s+default\s+(?:function\s+)?([A-Z][A-Za-z0-9]*)\b/.exec(readText(p));
      if (m) out.push(`export { default as ${m[1]} } from ${JSON.stringify(p)};`);
      return out;
    });
